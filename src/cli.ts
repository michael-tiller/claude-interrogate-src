...
async function main(): Promise<void> {
  ...
  const styleTemplatePath = valueAfter(args, "--style") ?? await resolveDefaultStyleTemplate(process.cwd());
  const outputPath =
    valueAfter(args, "--out") ?? path.join(docsDir, `${slugify(concept)}.md`);
...
function collectResponseFlags(args: string[]): Record<string, string> {
...
function needsFollowUp(questionId: string, answer: string): boolean {
...
function hasMeaningfulInspirations(value: string): boolean {
