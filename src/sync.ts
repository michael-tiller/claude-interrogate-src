...
export async function designCrossRefSync(docsDir: string, styleTemplatePath?: string): Promise<SyncReport> {
  ...
  for (const doc of docs) {
    const expectedLinks = docs
      ...
      .map(
        (candidate) =>
          `- [${candidate.title}](${toPosixRelative(path.dirname(doc.path), candidate.path)})`
      );

    let nextContent = doc.content;
    nextContent = replaceOrAppendSection(
      nextContent,
      style.crossRefHeading,
      expectedLinks.length ? expectedLinks.join("\n") : "- No sibling docs yet."
    );

    const openQuestions = extractOpenQuestions(nextContent, style.openQuestionsHeading);
    const existingResolved = extractBullets(getSectionBody(nextContent, "Resolved Decisions") ?? "").filter(
      (item) => !/^no resolved decisions/i.test(item)
    );

    const unresolvedQuestions: string[] = [];
    const resolvedAdditions: string[] = [];
    ...
    for (const question of openQuestions) {
      ...
      const insertionHeading = chooseInsertionHeading(
        nextContent,
        question,
        resolution,
        style
    ...
    if (resolvedDecisionItems.length > 0) {
      nextContent = replaceOrAppendSection(
        nextContent,
        "Resolved Decisions",
        resolvedDecisionItems.map((item) => `- ${item}`).join("\n")
      );
    } else {
...
function buildResolutionCandidates(
  ...
  return docs.flatMap((doc) =>
    listSections(doc.content)
      .filter(
        (section) =>
          section.heading.toLowerCase() !== openQuestionsHeading.toLowerCase() &&
...
function findResolution(
  ...
  let best: { candidate: ResolutionCandidate; score: number } | null = null;
  for (const candidate of candidates) {
    ...
    const headingBoost =
      /resolved decisions|decision|chosen shape|constraints|release bar/i.test(candidate.heading) ? 1 : 0;
...
function appendResolutionToSection(content: string, heading: string, resolutionLine: string): string {
...
function chooseInsertionHeading(
  content: string,
  question: string,
  resolution: ResolutionCandidate,
  style: { crossRefHeading: string; openQuestionsHeading: string }
): string | null {
  const sections = listSections(content).filter(
    (section) =>
      !equalsIgnoreCase(section.heading, style.crossRefHeading) &&
  ...
  for (const section of sections) {
    ...
    const score =
      countOverlap(questionTokens, headingTokens) +
