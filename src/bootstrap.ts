...
export async function bootstrapDocsSkeleton(docsDir: string, concept: string): Promise<string[]> {
...
- What should the first real decision for ${concept} be?`
  ...
  await Promise.all(
    files.map((file) => writeFile(path.join(docsDir, file.name), file.content, "utf8"))
  );
