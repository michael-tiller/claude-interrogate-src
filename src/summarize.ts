...
import { SummaryReport } from "./types.js";
...
export async function designSummarize(
  concept: string,
  docsDir: string,
  ...
  const learned = matches.flatMap(({ doc, matchedSections }) =>
    matchedSections.slice(0, 4).map((section) => `${doc.title}: ${section.heading} -> ${compact(section.body)}`)
  );

  const unresolved = matches.flatMap(({ doc, matchedSections }) =>
    matchedSections
