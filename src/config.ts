...
export async function loadInterrogateConfig(cwd: string): Promise<{
  path: string | null;
  ...
  for (const candidate of CONFIG_CANDIDATES) {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as InterrogateConfig;
      return {
        path: candidatePath,
...
export async function resolveDefaultDocsDir(cwd: string): Promise<string> {
...
export async function resolveDefaultStyleTemplate(cwd: string): Promise<string | undefined> {
  const { config, path: configPath } = await loadInterrogateConfig(cwd);
