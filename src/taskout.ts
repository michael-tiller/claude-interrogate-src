  ...
  if (
    ...
    const parts = [
      ...order.blockedByViolations.map(
        (v) =>
          `'${v.item}' is Blocked-by '${v.blocker}', which is listed at or after it — order the Targeted list so blockers come first`,
      ),
      ...order.unresolvedBlockedBy.map(
        (u) =>
 ...
 */
export function analyzeTaskoutOrder(
  sections: TaskoutExportSection[],
  rcId: string,
  raw?: string,
): OrderDiagnostics {
  ...
  for (const sub of sections) {
    for (const item of sub.items) {
      if (!indexOf.has(item.key)) indexOf.set(item.key, flatIndex);
      flatIndex += 1;
    }
  }

  const blockedByViolations: OrderDiagnostics["blockedByViolations"] = [];
  const unresolvedBlockedBy: OrderDiagnostics["unresolvedBlockedBy"] = [];
  for (const sub of sections) {
    for (const item of sub.items) {
  ...
  const strayOrderingSections: OrderDiagnostics["strayOrderingSections"] = [];
  if (raw) {
 ...
 */
function carryForwardKeys(
  existing: TargetedSubsection[],
  plan: TargetedSubsection[],
): void {
  ...
  const planByHeading = new Map<string, TargetedSubsection>();
  for (const sub of plan) {
    if (!planByHeading.has(sub.heading)) planByHeading.set(sub.heading, sub);
  }
