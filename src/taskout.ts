...
function buildTaskoutQuestions(
  ...
  questions.push({
    ...
    theme: "Targeted",
    question:
      "For each major area of work, list epic-level checklist items. Cite concept-doc sections inline.",
  ...
  questions.push({
    ...
    theme: "Per-item DOD",
    question:
      "For each Targeted item, give 1-3 observable pass/fail criteria that confirm it is done (rendered as `- DOD:` sub-bullets under the item). An item with no specific criteria inherits the milestone DoD.",
    rationale:
      "The board is the human-readable bridge from design to game; a concrete per-item requirement makes every downstream step (flay, verification, the ClickUp mirror) work against a spec, not just a title.",
  });
  if (techDebt.length || carried.length) {
    questions.push({
      ...
      question:
        "Confirm or edit the surfaced blockers (upstream RCs, scanned tech-debt items, carried-over items). Add any external pending decisions.",
