Maintain `scratch.md` — the project's intraday register of work in flight: triage existing entries, then prepend a dated section for this session.

`scratch.md` carries an unfinished task across a lunch break, the end of a work day, or a hand-off to another agent. It is not a roadmap, a changelog, or a tech-debt backlog. Items live here only while they are being actively worked.

Topic: treat `$ARGUMENTS` as the title for the new section. If empty, infer it from the dominant thread of the current conversation. Use a short noun phrase.

Resolve targets:
- Register file: `scratchFile` in `claude-interrogate.json` or `.claude-interrogate.json` if present (relative to the config file); otherwise `./scratch.md` at the project root.
- Triage cross-reference targets come from the `roadmap` block of `claude-interrogate.json`, the same keys the other commands use: `rcDir` (default `Roadmap`), `indexFile` (default `roadmap.md`), `techDebtFile` (default `Roadmap/TECHNICAL_DEBT.md`), `rcNamingScheme` (default `{prefix}{milestone}_{NAME}.md`). If the project has no roadmap or tech-debt files, skip the "defer to roadmap/tech-debt" disposition and only keep or remove.

Step 1 — triage existing entries. Read the full file. For every existing entry, pick exactly one disposition:
...
Step 2 — prepend the new section. Pull every fact from the actual conversation — do not invent file paths, line numbers, or hypotheses that were not discussed. Title format: `# <Topic> — YYYY-MM-DD`, using today's date. Prepend the new section above any surviving prior content, with a `---` rule between sections. Preserve the file-purpose header block at the top if one is present. If the file does not exist yet, create it, seed it with the purpose-header block below, then add the dated section beneath it. Print the path of the file you wrote.
