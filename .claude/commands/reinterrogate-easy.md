Run the reinterrogation flow for existing doc path `$1` in docs directory `$2` in easy mode.
...
- If MCP prompts are unavailable but the MCP tools are available, read the target document, then call `design_interview_start` with `challenge_mode="easy"` and `depth_mode="standard"` before running the reinterrogation.

Behavior:
- Keep the question queue private.
- Ask one question at a time, focusing on concrete deltas from current sibling knowledge.
