...
- Targeted granularity is epic-level checklist items, not story-level tasks. Detailed task breakdowns belong in `Plan/` docs.
...
- Coverage gate before write: every section has content; DoD has at least 3 testable assertions; References cites every doc named inline in Targeted.
