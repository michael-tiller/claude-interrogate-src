...
`claude-interrogate` now integrates with Claude Code in two direct ways:
...
After `npm install` and `npm run build`, Claude Code can attach the project MCP server from [.mcp.json](E:/Personal/claude-interrogate/.mcp.json).

That gives Claude Code:

- MCP tools
- MCP prompt slash commands
- repo-local project slash commands
...
The server now exposes MCP prompts, which Claude Code can discover as slash commands with normalized names:
...
- automatic Claude Code permission presets for the MCP tools
